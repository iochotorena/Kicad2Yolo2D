...
def _is_between_ccw(start: float, end: float, angle: float) -> bool:
...
def _arc_extents(
    ...
    center = _arc_center(start, mid, end)
    if center is None:
        return []
    ...
    ccw = _is_between_ccw(start_angle, end_angle, mid_angle)

    candidates = []
    for angle in (0.0, math.pi / 2, math.pi, 3 * math.pi / 2):
        in_sweep = _is_between_ccw(start_angle, end_angle, angle) if ccw else _is_between_ccw(end_angle, start_angle, angle)
        if in_sweep:
            candidates.append((center_x + radius * math.cos(angle), center_y + radius * math.sin(angle)))
    return candidates
...
def parse_pcb_dimensions(filepath: str | Path) -> PCBDimensions:
    ...
    for element in edge_elements:
        ...
        elif element.lstrip().startswith("(gr_arc") and start_matches and mid_matches and end_matches:
            start = (float(start_matches[0][0]), float(start_matches[0][1]))
            mid = (float(mid_matches[0][0]), float(mid_matches[0][1]))
            end = (float(end_matches[0][0]), float(end_matches[0][1]))
            all_points.extend(_arc_extents(start, mid, end))
